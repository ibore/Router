    ...
    public RouterResult intercept(Dispatcher dispatcher) {
        ...
        if (uri == null) {
            ...
            if (url.contains(":")) {
                ...
                url = XRouter.get().getSchemes().get(0) + "://" + XRouter.get().getDomains().get(0) + url;
                uri = Uri.parse(url);
            }
            action.setUri(uri);
        }
        ...
        if (target != null) {
            ...
            bundle.putString(XRouter.KEY_ORIGIN_URL, action.getOriginUrl());
            bundle.putString(XRouter.KEY_PATTERN, target.pattern);
